...
> Verzija: v1.3 | Datum: 27. jul 2026.
...
**Nezavršena sesija (`endTime: null`):** deli se u prikazu na dva stanja, u zavisnosti od toga da li `dateKey` odgovara današnjem danu:
- **"u toku"** — sesija je počela danas i još nema `endTime`; tretira se kao aktivan poziv, bez alarmantnog flaga
- **"nezavršeno"** — `endTime: null`, ali `dateKey` nije današnji dan; znak da je poziv ostao otvoren (crash/force quit), zahteva ručnu ispravku

U oba slučaja trajanje se ne nagađa — razlika je isključivo u prikazu (grouping.ts), ne u strukturi podataka. Korisnik ručno ispravi red koji je "nezavršeno".
...
| Nezavršena sesija (`endTime: null`) deli se u prikazu na "u toku" (danas, aktivna) i "nezavršeno" (stariji dan, zahteva ispravku) | Jedinstven "nezavršeno" flag za sve `endTime: null` slučajeve, bez razlike po danu | Aktivan poziv koji je još u toku ne treba da deluje kao greška; razlika odmah pokazuje šta zahteva ručnu intervenciju a šta je normalno stanje | Razlika se računa isključivo u prikazu (grouping.ts), struktura podataka se ne menja |
...
- **Verzija v1.3:** nezavršena sesija (`endTime: null`) sad se u prikazu deli na "u toku" (danas) i "nezavršeno" (stariji dan) — odluka potvrđena eksplicitno pre pisanja `grouping.ts` prompta.
